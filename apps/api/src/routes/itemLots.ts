...
export async function registerItemLotsRoutes(app: FastifyInstance, prisma: PrismaClient) {
  ...
  app.get('/item-lots', async (req, reply) => {
    ...
    if (q.search) where.lote_codigo = { contains: q.search };
    if (q.from || q.to) {
        where.fecha_ingreso = {};
        ...
        if (parsed.data.cantidad_inicial > 0) {
          await tx.inventory_movements.create({
            data: {
              item_id: parsed.data.item_id,
              lot_id: lot.id,
              fecha_hora: new Date(parsed.data.fecha_ingreso),  // ✅ Cambié "fecha" por "fecha_hora"
              tipo: 'IN',          // Movimiento de entrada
              motivo: 'COMPRA',    // Motivo por defecto
  ...
  // Stock por lote desde la VISTA (vw_stock_por_lote)
  // GET /item-lots/stock?item_id=&search=&page=&pageSize=
  app.get('/item-lots/stock', async (req, reply) => {
    const parsed = listItemLotsQuery.safeParse(req.query);
    if (!parsed.success) return reply.code(400).send(parsed.error.flatten());
    const q = parsed.data;

    // WHERE parametrizado (evita SQL injection)
    const whereClauses: string[] = [];
    const params: any[] = [];

    if (q.item_id) { whereClauses.push(`s.item_id = ?`); params.push(q.item_id); }
    if (q.search)  { whereClauses.push(`s.lote_codigo LIKE ?`); params.push(`%${q.search}%`); }

    const where = whereClauses.length ? `WHERE ${whereClauses.join(' AND ')}` : '';
    const order = `ORDER BY s.fecha_ingreso DESC, s.lote_codigo ASC`;

    const { skip, take } = toSkipTake(q);

    const data = await prisma.$queryRawUnsafe<any[]>(
      `
      SELECT s.item_id, s.item_nombre, s.item_unidad, s.lot_id, s.lote_codigo, s.fecha_ingreso, s.stock_actual
      FROM vw_stock_por_lote s
      ${where}
      ${order}
      LIMIT ? OFFSET ?
      `,
